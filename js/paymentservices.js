...
var PAYMENT =
    {
        ...
        provision(paymentInfo)
        {
            ...
            TRANSACTION.loginStatus = paymentInfo.loginStatus;
        ...
        cancelTransaction: function(tid, type){   // type can be cancel, shipment, subscription
            if(!tid) {
                UTILS.errorDetected("ERROR - Invalid transaction ID.\n");
                PAYMENT.completed();
                return;
            ...
            if(type=="unsubscribe"){
                PAYMENT.createAndSubmitToken(token, 2, STATUS.code.Unsubscribe);
            } else {
                PAYMENT.createAndSubmitToken(token, 2, STATUS.code.Cancel);
            }
        },
