            ...
            success     : function(hmac) {
                ...
                $.ajax({
                    ...
                    success     : function(result) { 
                        ...
                        if ((emailChangeResp.status === STATUS.code.MACVerificationFailure) ||
                                 (emailChangeResp.status === STATUS.code.VIDFailure))
                        {
                            UTILS.errorDetected("ERROR - Email Change failure: " + emailChangeResp.toString());  
                            return;
                        }
                        
                        CARDHOLDER.id = anotherVid;
                    },
                    error: function()
                    {
                        UTILS.errorDetected("ERROR - Unexpected email change response.\n");  
            ...
            function (hmac) 
            {
                ...
                { 
                        ...
                        (prePaymentResp.status === STATUS.code.AnotherVID))
                    {
                        ...
                        {
                            // 303a:  Prompt user for choice of other available vid, email
                            var anotherVID = window.confirm("Would you like to use an existing email, " + 
                                                             prePaymentResp.anotherEmail + 
                                                            ", that registered with mobile number, " +
                                                            CARDHOLDER.phone + "?\n");
                            if(anotherVID) {
